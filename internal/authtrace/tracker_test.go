...
func TestUnregisterContract_Existing(t *testing.T) {
	...
	if len(v.ListContracts()) != 0 { t.Error("expected empty list after unregister") }
}
...
func TestGetContractInfo_RecoveryHandler(t *testing.T) {
	...
	if err != nil { t.Fatalf("unexpected error: %v", err) }
	if info["auth_name"] != "recovery" { t.Errorf("expected recovery, got %v", info["auth_name"]) }
...
func TestMultiSigValidation_NoParams(t *testing.T) {
	...
	if valid || err == nil { t.Error("expected failure with no params") }
}
func TestMultiSigValidation_InvalidParamType(t *testing.T) {
	auth := NewMultiSigContractAuth(1, 1, nil)
	valid, err := auth.ValidateAuth("c", "m", []interface{}{"not-a-map"})
	if valid || err == nil { t.Error("expected failure for non-map param") }
}
func TestMultiSigValidation_MissingSignaturesField(t *testing.T) {
	...
	if valid || err == nil { t.Error("expected failure for missing signatures field") }
}
func TestMultiSigValidation_UnknownSignerKeys(t *testing.T) {
	...
	if valid { t.Error("expected failure for unknown signer") }
...
func TestMultiSigValidation_PartialWeightBelowThreshold(t *testing.T) {
	...
	if valid, _ := auth.ValidateAuth("c", "m", params); valid { t.Error("expected failure: weight below threshold") }
}
func TestMultiSigValidation_ExactThreshold(t *testing.T) {
	auth := NewMultiSigContractAuth(2, 5, map[string]uint32{"key1": 3, "key2": 2})
	params := []interface{}{map[string]interface{}{"signatures": []interface{}{map[string]interface{}{"signer_key": "key1"}, map[string]interface{}{"signer_key": "key2"}}}}
	valid, err := auth.ValidateAuth("c", "m", params)
	if !valid { t.Errorf("expected success at exact threshold: %v", err) }
}
func TestMultiSigValidation_DuplicateSignerEntries(t *testing.T) {
	...
	if valid, _ := auth.ValidateAuth("c", "m", params); !valid { t.Error("expected success with duplicate weight accumulation") }
}
func TestMultiSigValidation_NonMapSignatureEntries(t *testing.T) {
	...
	valid, err := auth.ValidateAuth("c", "m", params)
	if !valid { t.Errorf("expected success after skipping bad entry: %v", err) }
}
func TestMultiSigDetails_ContainsTotalSigners(t *testing.T) {
	auth := NewMultiSigContractAuth(2, 4, map[string]uint32{"a": 1, "b": 2, "c": 3})
	details := auth.GetAuthDetails()
	if details["total_signers"] != 3 { t.Errorf("expected 3, got %v", details["total_signers"]) }
	if details["signer_threshold"] != uint32(4) { t.Errorf("expected 4, got %v", details["signer_threshold"]) }
}
func TestNewMultiSigContractAuth_NilSigners(t *testing.T) {
	if NewMultiSigContractAuth(1, 1, nil).Signers == nil { t.Error("expected non-nil Signers map") }
}
func TestRecoveryAuth_InsufficientParams(t *testing.T) {
	r := NewRecoveryAuth("key", 0)
	valid, err := r.ValidateAuth("c", "m", []interface{}{"key"})
	if valid || err == nil { t.Error("expected failure with 1 param") }
}
func TestRecoveryAuth_InvalidKeyType(t *testing.T) {
	r := NewRecoveryAuth("key", 0)
	valid, err := r.ValidateAuth("c", "m", []interface{}{12345, nil})
	if valid || err == nil { t.Error("expected failure for non-string key") }
}
func TestRecoveryAuth_EmptyKeyMatch(t *testing.T) {
	r := NewRecoveryAuth("", 0)
	valid, err := r.ValidateAuth("c", "m", []interface{}{"", nil})
	if !valid { t.Errorf("expected success for matching empty keys: %v", err) }
}
func TestRecoveryAuthDetails(t *testing.T) {
	details := NewRecoveryAuth("my_key", 5000).GetAuthDetails()
	if details["recovery_key"] != "my_key" { t.Errorf("expected my_key, got %v", details["recovery_key"]) }
	if details["delay_ms"] != uint64(5000) { t.Errorf("expected 5000, got %v", details["delay_ms"]) }
}
func TestNestedAuth_MultipleContractsInSameValidator(t *testing.T) {
	...
	if !valid { t.Errorf("multi-sig failed: %v", err) }
	valid, err = v.ValidateContract("rc", "recover", []interface{}{"recover_me", nil})
	if !valid { t.Errorf("recovery failed: %v", err) }
}
func TestNestedAuth_SequentialValidation(t *testing.T) {
	...
	if ok, err := v.ValidateContract("gate", "unlock", []interface{}{"gate_key", nil}); !ok { t.Fatalf("gate failed: %v", err) }
	...
	if !valid { t.Errorf("action failed after gate passed: %v", err) }
}
func TestNestedAuth_FailAtGate(t *testing.T) {
	v := NewCustomContractAuthValidator()
	_ = v.RegisterContract("gate", NewRecoveryAuth("correct_key", 0))
	if ok, _ := v.ValidateContract("gate", "unlock", []interface{}{"wrong_key", nil}); ok { t.Fatal("gate should reject wrong key") }
}
func TestNestedAuth_ContractReplacementMidFlow(t *testing.T) {
	...
	if ok, _ := v.ValidateContract("auth", "m", params); !ok { t.Fatal("expected success with weak auth") }
	_ = v.RegisterContract("auth", NewMultiSigContractAuth(1, 100, map[string]uint32{"k": 1}))
	if ok, _ := v.ValidateContract("auth", "m", params); ok { t.Error("expected failure after strict replacement") }
}
func TestUnmarshalCustomContractAuth_ValidMultiSig(t *testing.T) {
	...
	if err != nil { t.Fatalf("unexpected error: %v", err) }
	if len(v.ListContracts()) != 1 { t.Errorf("expected 1, got %d", len(v.ListContracts())) }
}
...
func TestUnmarshalCustomContractAuth_UnknownType(t *testing.T) {
	if _, err := UnmarshalCustomContractAuth([]byte(\`{"c1":{"type":"unknown_type"}}\`)); err == nil { t.Error("expected error") }
}
func TestUnmarshalCustomContractAuth_MissingType(t *testing.T) {
	if _, err := UnmarshalCustomContractAuth([]byte(\`{"c1":{"required_signatures":1}}\`)); err == nil { t.Error("expected error") }
}
func TestUnmarshalCustomContractAuth_InvalidJSON(t *testing.T) {
	if _, err := UnmarshalCustomContractAuth([]byte(\`not-json\`)); err == nil { t.Error("expected error") }
}
func TestUnmarshalCustomContractAuth_MultipleContracts(t *testing.T) {
	...
	if err != nil { t.Fatalf("unexpected error: %v", err) }
	if len(v.ListContracts()) != 2 { t.Errorf("expected 2, got %d", len(v.ListContracts())) }
}
func TestTrackerRecordCustomContractCall_Success(t *testing.T) {
	...
	trace := tracker.GenerateTrace()
	if len(trace.AuthEvents) != 1 { t.Fatalf("expected 1 event, got %d", len(trace.AuthEvents)) }
	if trace.AuthEvents[0].EventType != "custom_contract_auth" { t.Errorf("wrong type: %s", trace.AuthEvents[0].EventType) }
	if trace.AuthEvents[0].ErrorReason != "" { t.Errorf("expected no error reason, got: %s", trace.AuthEvents[0].ErrorReason) }
}
...
func TestTrackerGetAuthEvents_FiltersByAccount(t *testing.T) {
	...
	tracker.RecordEvent(AuthEvent{EventType: "e", AccountID: "A", Status: "valid"})
	if events := tracker.GetAuthEvents("A"); len(events) != 2 { t.Errorf("expected 2, got %d", len(events)) }
}
func TestTrackerGetFailureReport_ReturnsNilForUnknown(t *testing.T) {
	if NewTracker(Config{}).GetFailureReport("UNKNOWN") != nil { t.Error("expected nil") }
}
func TestTrackerMaxEventDepth_IgnoresOverflow(t *testing.T) {
	tracker := NewTracker(Config{MaxEventDepth: 2})
	for i := 0; i < 5; i++ { tracker.RecordEvent(AuthEvent{EventType: "e", Status: "valid"}) }
	if trace := tracker.GenerateTrace(); len(trace.AuthEvents) != 2 { t.Errorf("expected 2, got %d", len(trace.AuthEvents)) }
}
